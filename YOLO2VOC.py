...
## custom classes
YOLO_CLASSES = ('date', 'price', 'company')
...
## define path
ROOT = './'
ANN_DIR = 'annotations_yolo'
IMG_DIR = 'images'
OUTPUT_DIR = 'annotations_pascal'
EXTENTION = '.png'


## converts coco into xml 
def xml_transform(root, classes, ann_dir, img_dir, output_dir, ext):  
    # class_path  = join(root, 'labels')
    class_path  = join(root, ann_dir)
    ids = list()
    ...
    annopath = join(root, ann_dir, '%s.txt')
    imgpath = join(root, img_dir, '%s{}'.format(ext))
    
    os.makedirs(join(root, output_dir), exist_ok=True)
    outpath = join(root, output_dir, '%s.xml')
    ...
    for index in range(len(ids)):
        ...
        f.write(xml)
        f.close()
        print(f"Complete to convert [{index+1:04}/{len(ids):04}]")
       

xml_transform(ROOT, YOLO_CLASSES, ANN_DIR, IMG_DIR, OUTPUT_DIR, EXTENTION)